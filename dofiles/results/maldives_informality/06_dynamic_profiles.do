...
keep year hhid pid fexp_* sample male urban age* h_head formal_* informal_* h_size depen welfare_* lp_*usd_* poor*1 tot_lai_* pc_*_s active_* emplyd* unemplyd*
...
forvalues i = `init' / `end' {
	...
	foreach kind of local categories {
		...
		*qui gen pg_`kind'`i' = pg`i' if `kind'`i' == 1
		...
		qui gen hh_for_`kind'`i' = formal_s`i' if `kind'`i' == 1 & h_head == 1
		qui gen hh_inf_`kind'`i' = informal_s`i' if `kind'`i' == 1 & h_head == 1
		...
		qui gen for_`kind'`i' = formal_s`i' if `kind'`i' == 1
		qui gen inf_`kind'`i' = informal_s`i' if `kind'`i' == 1
	...
	* Data collapse
	...
	preserve
	qui collapse (sum) pop* (mean) *_s_* /*pg**/ urban_* h_size_* ti_* li_* nli_* hh_* pub_tr_* priv_tr_* age*_* depend_* male_* female_* for_* inf_* emp_* unemp_* pensions_* capital_* otherinla_* renta_imp_* [iw = fexp_s`i']
	keep *`i'
	ren *`i' *
