...
while (`mm' > 0) {
	...
	if "$country" == "MDV" & $year == 2019 & `mm' <= 14 loc mm = 0	
	else if `mm' <= 6 loc mm = 0	
	note: adjust in case it doesn´t converge, increase the mm. 
}
...
if $m == 1 {
	...
	* Creates the structure sectoral matrix - targets
	...
	forvalues a = 1/`lim' {
		sum informal [aw = weight] if lf_samp == 1 & emplyd == 1 & sect_main == `a' 
		mat informal[`a',1] = r(mean)*(1 + growth_intrasectoral[`a',1])
	}

	* Allocate individuals according to their probability of being informal
...
if $m != 1 {
	...
	gen  	informal_s = 1 if inlist(sect_main6_s,2,4,6) & emplyd_s == 1
	replace informal_s = 0 if inlist(sect_main6_s,1,3,5) & emplyd_s == 1
}
...
gen formal_s = !informal_s if informal_s != .
