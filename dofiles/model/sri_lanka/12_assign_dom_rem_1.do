...
mtab1 region urban [aw = fexp_s] if h_head == 1 & h_dom_remit > 0 & h_dom_remit < ., sum(h_dom_remit) matrix(RT) med(MED) quanty(REM) total // Mean, median, and weight
mtab1 region urban [aw = fexp_s] if h_head == 1, sum(h_dom_remit) matrix(tp) quanty(TH)

* Target growth rate
mat growth_dom_rem = growth_capital[1,1]

* Increasing order of growth rates of domestic remittances 
mata: st_order("growth_dom_rem", 1, 2, "aux_rem")
loc r = rowsof(growth_dom_rem)
sca r = rowsof(growth_dom_rem)

forvalues i = 1(1)`r' {
	
	* Growth rate according to population growth 
	sum h_dom_remit [w = weight] if h_dom_remit > 0 & h_dom_remit < .
	sca var0 		= r(sum) / 1000000
	sum h_dom_remit [w = fexp_s] if h_dom_remit > 0 & h_dom_remit < .
...
forvalues i = 1(1)`r' {
		...
		* Growth rate according to population growth 
		...
		if scalar(Tr) > 0 {
			...
			* New remittances variable
			loc 	w = aux_rem_1[`i',1]
			egen    h_dom_remit_`w' = rsum(h_dom_remit aux2)
			replace h_dom_remit_`w' = . if h_dom_remit == . & aux2 == .

			* Correction & check
			sum h_dom_remit 	[w = weight] if h_dom_remit		> 0 & h_dom_remit 	  <.
			sca var0 			= r(sum)
			sum h_dom_remit_`w' [w = fexp_s] if h_dom_remit_`w' > 0 & h_dom_remit_`w' <.
			sca var1 			= r(sum)
			sca g_in 			= aux_rem_1[`i',2]
			mata: st_corr2("var0","var1","g_in")
			replace h_dom_remit_`w' = h_dom_remit_`w' *(1 + growth_inla_n[1,1]) if h_dom_remit_`w' > 0 & h_dom_remit_`w' <.

			sum h_dom_remit_`w' [aw = fexp_s] if h_dom_remit_`w' > 0 & h_dom_remit_`w' <.
			sca var1 			= r(sum)
		...
		if scalar(Tr) < 0 {
			...
			* New remittances according to the new growth rate only to those who had it
			loc 	w = aux_rem_1[`i',1]
			gen     h_dom_remit_`w' = h_dom_remit
			replace h_dom_remit_`w' = h_dom_remit_`w' *(1 + growth_inla_n[1,1]) if h_dom_remit_`w' > 0 & h_dom_remit_`w' <.
	...
	if `i' >= 2 {
		...
		* Growth rate of remittances from abroad according to population growth & previous remittances growth
		sum h_dom_remit 	[aw = weight] if h_dom_remit 	 > 0 & h_dom_remit     < .
		sca var0 			= r(sum)
		sum h_dom_remit_`j' [aw = fexp_s] if h_dom_remit_`j' > 0 & h_dom_remit_`j' < .
		sca var1 			= r(sum)
		sca mean 			= r(mean)
		sum h_head 			[aw = fexp_s] 
		sca N 				= r(sum)
		...
		if scalar(Tr) > 0 {
			...
			* New remittances variable
			loc 	w = aux_rem_1[`i',1]
			egen    h_dom_remit_`w' = rsum(h_dom_remit_`j' aux2)
			replace h_dom_remit_`w' = . if h_dom_remit_`j' == . & aux2 == .
...
loc m = scalar(v)
clonevar h_dom_remit_s = h_dom_remit_`m'
